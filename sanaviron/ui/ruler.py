    ...
    def motion(self, widget, event, external):
        if self.orientation == HORIZONTAL:
            ...
            for tag in self.tags:
                if tag.selected and event.get_state() & Gdk.ModifierType.BUTTON1_MASK:
                    tag.move(self.x)
                    self.emit("move-tag", tag)
                elif tag.at_position(self.x):
                    widget.window.set_cursor(Gdk.Cursor.new(tag.get_cursor()))
                    self.show_position = False
                    break
                else:
                    widget.window.set_cursor(Gdk.Cursor.new(Gdk.CursorType.ARROW))
                    self.show_position = True
        elif self.orientation == VERTICAL:
